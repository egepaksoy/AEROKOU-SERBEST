...
def adjust_x(orta_orani, x_fov, vehicle: Vehicle, DRONE_ID: int, detected_obj: dict, detected_obj_lock: threading.Lock, stop_event: threading.Event):
...
def adjust_y(orta_orani, y_fov, gimbal_channel, vehicle: Vehicle, DRONE_ID: int, detected_obj: dict, detected_obj_lock: threading.Lock, stop_event: threading.Event):
...
def go_to_obj(vehicle: Vehicle, DRONE_ID, config: dict, detected_obj: dict, detected_obj_lock: threading.Lock, stop_event: threading.Event):
    ...
    x_centered = adjust_x(orta_orani, x_fov, vehicle, DRONE_ID, detected_obj, detected_obj_lock, stop_event)
    ...
    while not stop_event.is_set():
        ...
        if (y_dist == 0 or y_dist < 0) and (current_servo_pwm >= gimbal_angles[0] - 200 and current_servo_pwm <= gimbal_angles[0]):
            ...
            if adjust_y(orta_orani, y_fov, gimbal_channel, vehicle, DRONE_ID, detected_obj, detected_obj_lock, stop_event) == False:
...
def go_to_obj(vehicle: Vehicle, DRONE_ID, orta_orani: float, gimbal_channel: int, gimbal_angles: list, fov: list, detected_obj: dict, detected_obj_lock: threading.Lock, stop_event: threading.Event):
    ...
    while not stop_event.is_set():
        ...
        if x_dist != 0:
            if adjust_x(orta_orani, x_fov, vehicle, DRONE_ID, detected_obj, detected_obj_lock, stop_event) == False:
